<template>
  <div class="WechatEmotion" v-show="isShow" @click.self="close">
    <div>
        <i v-for="(item,index) in list" @click="check(index)">
...
import { WechatEmotion as Emotion } from 'vux'
  export default {
    ...
    methods:{
      init(){},
      close(){
        this.$emit('selectEmotion',false)
      },
      check(i){
        //console.log(i)
        this.close()
        this.$emit('checkVal',[this.list[i],i])
      },
    },
    computed: {},
    components:{Emotion},
