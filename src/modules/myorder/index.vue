      ...
      @top-status-change="handleTopChange">
        ...
        <div v-infinite-scroll="getNextList" infinite-scroll-disabled="loadingBottom" infinite-scroll-distance="80">
          <ul>
            ...
            <li :class="['clearfix']" v-for="(item,i) in orderList" :key="i" @click="gotoDetail(item.orderCode)">
              <section>
                <div >
                    <div>
                    ...
                    <div style="line-height:45px;border-bottom: 1px solid #E2E2E2;">
                        <span style="font-size:12px;padding: 2px;border-radius: 2px;" 
                          :class="{'border-color-blue':item.orderType=='订金支付','border-color-grey':item.orderType=='维修订单','border-color-navy':item.orderType=='保养订单'}">{{item.orderType}}</span>
                        <span style="color:#8D8D8D;float:right;font-size:12px;">创建时间：{{item.createTime}}</span>
                    </div>
                    <div style="font-size:15px;margin-top:20px;margin-bottom:4px;">
                        <span >林肯中心：{{item.dealerName}}</span>
                        <span style="float:right;margin-right:1%;line-height: 60px;margin-top: -17px;font-size:14px;" >{{item.tradeStatus}}</span>
                    </div>
                  
                </div>
                <div style="font-size:15px;margin-bottom:20px">
                  <div >订单金额：￥{{item.deposit|priceFormat}}</div>
                </div>
              </section>
